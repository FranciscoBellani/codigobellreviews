...
function cwr_custom_review_form($comment_form) {
    ...
    $comment_form['comment_field'] .= '<p><strong>Valoraciones Específicas</strong> <small>(Gana 10 puntos por completar todas las características)</small></p>';
    foreach ($ratings_array as $rating) {
        ...
        $comment_form['comment_field'] .= '
            <p class="comment-form-' . esc_attr($rating_key) . '">
                ...
                <small>(Gana 3 puntos por esta valoración)</small>
            </p>';
    }
    $comment_form['comment_field'] .= '</div>';

    // Sección Me Gusta / No Me Gusta
    ...
    $comment_form['comment_field'] .= '
        <p class="comment-form-like-dislike">
            ...
            <small>(Gana 3 puntos por seleccionar)</small>
        </p>';

    // Sección Compraste / Lo Voy a Comprar
    ...
    $comment_form['comment_field'] .= '</select>
            <small>(Gana 5 puntos por seleccionar)</small>
        </p>';
    ...
    $comment_form['comment_field'] .= '
        <p class="comment-form-feature">
            ...
            <small>(Gana 10 puntos por sugerir)</small>
        </p>';

    // Aquí agregamos el campo nonce para seguridad
    $comment_form['comment_field'] .= wp_nonce_field('cwr_save_review_meta', 'cwr_nonce', true, false);

    // Scripts y estilos aplicados al formulario
    ...
    $comment_form['comment_field'] .= '
        <style>
            ...
            small { color: #666; font-size: 0.8em; display: block; margin-top: 5px; }
        </style>
...
// Carga jQuery y dashicons sólo en página de producto
